...
<p align="center">
  <a href="en/plugin-development-guide.md">English</a> | <strong>简体中文</strong>
</p>
...
### 3.5 高级：开发独立设置弹窗 (Settings Window)

如果插件需要独立的配置界面（通过控制面板插件卡片上的齿轮按钮唤出），请遵循统一的设置弹窗规范：

1. **实现 `Plugin::build_settings_window`**：
   使用 `widget_core::default_settings_window_options` 创建标准弹窗：
   ```rust
...
2. **使用公共设置外壳 `render_settings_shell`**：
   设置页面**必须**使用 `render_settings_shell` 包装，自动获得统一的标题栏原生拖拽、右上角关闭按钮及内嵌滚动容器：
   ```rust
   ...
   impl Render for MySettingsView {
       fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
           let content = div()
               ...
               .gap(px(12.0))
               // 区块小标题
               .child(settings_section_header("通用设置"))
               // 标准卡片容器
               .child(
                   settings_card()
                       .p(px(14.0))
           ...
           render_settings_shell("我的插件 - 设置", content)
       }
   }
   ```
