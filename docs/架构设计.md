...
<p align="center">
  <a href="en/architecture.md">English</a> | <strong>简体中文</strong>
</p>
...
* **动态组件 (Dynamic Plugin Widgets)**：通过官方 `gpui-shell` 引擎加载执行的第三方 JavaScript 扩展（ES Modules + `View` 类），底层直接映射到 GPUI GPU 渲染管线，享有与内置组件同等的 DirectComposition 亚克力透明与 `Progman` 常驻能力。

### 2.2 Controller Layer (控制层)
基于 Rust 的核心业务运行枢纽。
* **App Core Manager & Window Manager**：核心事件调度与多窗口句柄管理。
* **Plugin & Extension Manager**：插件生命周期管理器。负责内置原生插件与外部 JS 扩展的发现、扫描加载、热重新载入（Reload）。
* **GPUI Shell 脚本执行沙箱 (`gpui-shell`)**：基于嵌入式 QuickJS JIT 虚拟机的 JavaScript 运行时。内置现代 ES Module 解析器、DOM-free 响应式状态通知 (`cx.notify()`)、原生定时器 (`cx.timer.every`) 与 GPUI 样式反射系统。隔离第三方逻辑并桥接宿主能力。
