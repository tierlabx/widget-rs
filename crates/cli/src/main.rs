...
enum Commands {
    ...
    /// 版本发布：自动计算版本号、更新 Cargo.toml、生成 CHANGELOG、提交并推送
    Release {
        /// 手动指定版本号（如 0.2.0），不指定则根据 Conventional Commits 自动计算
        #[arg(long)]
            ...
            match action {
                PluginAction::Add { name, path } => {
                    ...
                    if doc["dependencies"].get(name).is_some() {
                        println!("插件 '{}' 已存在于 Cargo.toml 中。", name);
                    } else {
                        if let Some(p) = path {
                    ...
                    let mut registry_code = fs::read_to_string(&plugin_registry_rs)
                        .expect("无法读取 plugin_registry.rs");
                    let inject_line = format!("    pm.register({}::create_plugin());\n", name);

                    if registry_code.contains(&inject_line) {
                        println!("插件 '{}' 已在 plugin_registry.rs 中注册。", name);
                    } else if let Some(pos) =
                        registry_code.find("// [WIDGET_CLI_INJECT_PLUGINS_END]")
                    {
                ...
                PluginAction::Remove { name } => {
                    ...
                    if doc["dependencies"].get(name).is_some() {
                        ...
                        println!("在 Cargo.toml 中未找到插件 '{}'", name);
                    }
                    ...
                    if registry_code.contains(&inject_line) {
                        ...
                        println!("在 plugin_registry.rs 中未找到插件 '{}' 的注册代码", name);
                    }
                }
            }
        }
        Commands::Release { version, dry_run } => {
            release::run_release(&workspace_root, version.as_deref(), *dry_run);
        }
    }
}
