...
impl WindowManager {
    ...
    pub fn save_all_plugin_bounds(&self, cx: &mut App, store: &Store) {
        ...
        for (id, (handle, hwnd, _owner)) in &self.widget_windows {
            ...
            if let Ok(bounds) = handle.update(cx, |_, window, _| window.bounds()) {
                ...
                // 逻辑坐标由 get_window_bounds 计算（GetWindowRect / actual_scale），
                // 与 resolve_plugin_bounds 的 phys/target_scale 公式完全对称：
                // 保存：log = GetWindowRect / actual_scale
                // 还原：GPUI px(log) × target_scale → 回到原始物理坐标
                entry.x = x;
                entry.y = y;
                entry.width = width;
                entry.height = height;
                ...
                println!(
                    ...
                    id, entry.x, entry.y, width, height, actual_scale,
    ...
    /// 将所有离屏或超出工作区的插件窗口归位到最近的有效显示器工作区内
    pub fn reposition_all_to_valid_screens(&self, cx: &mut App) {
        crate::window::reposition::reposition_all_to_valid_screens(&self.widget_windows, cx);
    }
}
