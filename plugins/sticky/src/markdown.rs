...
pub fn render_markdown(text: &str) -> impl IntoElement {
        ...
        match event {
            ...
            Event::End(TagEnd::Heading(_)) => {
                let mut d = div().w_full().mb(px(8.0)).child(
                    div().text_color(rgb(0x3d2000)).font_weight(FontWeight::BOLD).child(current_text.clone())
                );
            ...
            Event::End(TagEnd::Paragraph) => {
                if !current_text.is_empty() {
                    let mut d = div().w_full().mb(px(4.0)).text_sm().text_color(rgb(0x3d2000)).child(current_text.clone());
                    if is_quote {
                        d = d.border_l_4().border_color(rgba(0xf59e0b80)).pl(px(8.0)).text_color(rgb(0x78350f));
                    }
                    elements.push(d.into_any_element());
                    current_text.clear();
                }
            ...
            Event::Start(Tag::Image { dest_url, .. }) => {
                ...
                if url.starts_with("lmdb://") {
                    ...
                    if let Some(db) = crate::db::global_db() {
                        if let Some(bytes) = db.get_image(&key) {
                            ...
                            if let Some(path_str) = temp_file.to_str() {
                                ...
                                elements.push(
                                    div().w_full().mb(px(8.0)).child(
                                        img(SharedString::from(image_path)).w_full().rounded(px(4.0))
                                    ).into_any_element()
                                );
                            }
                        }
                    ...
                    elements.push(
                        div().w_full().mb(px(8.0)).child(
                            img(SharedString::from(url)).w_full().rounded(px(4.0))
                        ).into_any_element()
                    );
                }
            }
    ...
    // 如果结尾还有未提交的文本
    if !current_text.is_empty() {
        elements.push(div().w_full().text_sm().text_color(rgb(0x3d2000)).child(current_text.clone()).into_any_element());
    }
